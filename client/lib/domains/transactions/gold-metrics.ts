  ...
  // ============================================================================
  {
    ...
    sqlDefinition: `
      WITH all_transactions AS (
        ...
        UNION ALL
        SELECT TRANSACTION_DATE, TRANSACTION_ID, 'HOLD' as TXN_TYPE FROM CORE_DEPOSIT.FCT_DEPOSIT_HOLD_TRANSACTION WHERE HOLD_STATUS = 'ACTIVE'
        UNION ALL
        SELECT TRANSACTION_DATE, TRANSACTION_ID, 'MAINTENANCE' as TXN_TYPE FROM CORE_DEPOSIT.FCT_DEPOSIT_MAINTENANCE_TRANSACTION WHERE MAINTENANCE_STATUS = 'COMPLETED'
        UNION ALL
        SELECT TRANSACTION_DATE, TRANSACTION_ID, 'STOP' as TXN_TYPE FROM CORE_DEPOSIT.FCT_DEPOSIT_STOP_TRANSACTION WHERE STOP_STATUS = 'ACTIVE'
      ),
      daily_volumes AS (
        SELECT
