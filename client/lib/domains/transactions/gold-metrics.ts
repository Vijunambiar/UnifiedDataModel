  ...
  },
  {
    ...
    sqlDefinition: `
      WITH all_transactions AS (
        ...
        UNION ALL
        SELECT HOLD_START_DATE as TRANSACTION_DATE, HOLD_AMOUNT as TRANSACTION_AMOUNT FROM CORE_DEPOSIT.FCT_DEPOSIT_HOLD_TRANSACTION WHERE HOLD_STATUS = 'ACTIVE'
        UNION ALL
        SELECT CHANGE_DATE as TRANSACTION_DATE, 0 as TRANSACTION_AMOUNT FROM CORE_DEPOSIT.FCT_DEPOSIT_MAINTENANCE_TRANSACTION
        UNION ALL
        SELECT STOP_ENTERED_DATE as TRANSACTION_DATE, STOP_AMOUNT as TRANSACTION_AMOUNT FROM CORE_DEPOSIT.FCT_DEPOSIT_STOP_TRANSACTION WHERE STOP_STATUS = 'ACTIVE'
      )
      SELECT
        CAST(TRANSACTION_DATE AS DATE) as txn_date,
  ...
  },
  {
    ...
    sqlDefinition: `
      WITH all_transactions AS (
        ...
        UNION ALL
        SELECT 'HOLD' as txn_type, COUNT(*) FROM CORE_DEPOSIT.FCT_DEPOSIT_HOLD_TRANSACTION WHERE HOLD_START_DATE = CURRENT_DATE() AND HOLD_STATUS = 'ACTIVE'
        UNION ALL
        SELECT 'MAINTENANCE' as txn_type, COUNT(*) FROM CORE_DEPOSIT.FCT_DEPOSIT_MAINTENANCE_TRANSACTION WHERE CHANGE_DATE = CURRENT_DATE()
        UNION ALL
        SELECT 'STOP' as txn_type, COUNT(*) FROM CORE_DEPOSIT.FCT_DEPOSIT_STOP_TRANSACTION WHERE STOP_ENTERED_DATE = CURRENT_DATE() AND STOP_STATUS = 'ACTIVE'
      ),
      total_count AS (
  ...
  },
  {
    ...
    sqlDefinition: `
      WITH all_transactions AS (
        ...
        UNION ALL
        SELECT TRANSACTION_ID FROM CORE_DEPOSIT.FCT_DEPOSIT_HOLD_TRANSACTION WHERE MONTH(HOLD_START_DATE) = MONTH(CURRENT_DATE()) AND YEAR(HOLD_START_DATE) = YEAR(CURRENT_DATE()) AND HOLD_STATUS = 'ACTIVE'
        UNION ALL
        SELECT TRANSACTION_ID FROM CORE_DEPOSIT.FCT_DEPOSIT_MAINTENANCE_TRANSACTION WHERE MONTH(CHANGE_DATE) = MONTH(CURRENT_DATE()) AND YEAR(CHANGE_DATE) = YEAR(CURRENT_DATE())
        UNION ALL
        SELECT TRANSACTION_ID FROM CORE_DEPOSIT.FCT_DEPOSIT_STOP_TRANSACTION WHERE MONTH(STOP_ENTERED_DATE) = MONTH(CURRENT_DATE()) AND YEAR(STOP_ENTERED_DATE) = YEAR(CURRENT_DATE()) AND STOP_STATUS = 'ACTIVE'
      )
